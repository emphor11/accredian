...
export function Navbar() {
  ...
  const navLinkClass = (id: string) =>
    `border-b-[3px] pb-1 text-base font-bold transition ${
      activeId === id
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 bg-white shadow-[0_3px_12px_rgba(15,23,42,0.14)]">
      <nav className="section-shell flex h-[86px] items-center justify-between gap-8">
        <Link href="#home" className="shrink-0" aria-label="Accredian home">
          ...
          <span className="block text-xs font-medium tracking-[0.28em] text-gray-500">
        ...
        <button
          ...
          className="inline-flex h-11 w-11 items-center justify-center rounded-md border border-gray-200 text-gray-900 xl:hidden"
        >
        ...
        <div className="section-shell grid gap-1 py-5">
          {navItems.map((item) => (
            <a
              ...
              className="rounded-md px-3 py-3 text-base font-bold text-gray-900 hover:bg-blue/10 hover:text-blue"
            >
              {item.label}
            </a>
          ))}
